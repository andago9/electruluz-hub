...
export const Route = createFileRoute("/contacto")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Visítanos en Calle 11 #7-52, La Dorada. WhatsApp +57 312 288 1095." },
    ],
  }),
  component: Contacto,
});
    ...
    <>
      <section className="bg-gradient-hero text-white py-20">
        <div className="mx-auto max-w-7xl px-4 lg:px-8">
          <span className="text-sm font-bold uppercase tracking-widest" style={{ color: "var(--brand-yellow)" }}>Contacto</span>
          <h1 className="mt-2 text-4xl md:text-6xl font-extrabold">Estamos para <span className="text-gradient-fire">ayudarte</span></h1>
          <p className="mt-6 max-w-2xl text-lg text-white/80">Visítanos, llámanos o escríbenos por WhatsApp. Atención inmediata.</p>
        </div>
      </section>
        ...
        <div className="mx-auto max-w-7xl px-4 lg:px-8 grid gap-10 lg:grid-cols-2">
          <div className="space-y-4">
            {[
              ...
              <div key={x.t} className="flex gap-4 rounded-2xl border border-border bg-card p-5 shadow-card">
                ...
                <div><div className="font-extrabold">{x.t}</div><div className="text-muted-foreground">{x.d}</div></div>
              </div>
            ))}
            <div className="rounded-2xl border border-border bg-card p-5 shadow-card">
              ...
              <ul className="space-y-1.5 text-sm">
                {SITE.hours.map(h => (
                  <li key={h.d} className="flex justify-between border-b border-border last:border-0 pb-1.5">
          ...
          <form onSubmit={onSubmit} className="rounded-2xl border border-border bg-card p-6 shadow-card space-y-4" noValidate>
            <h2 className="text-2xl font-extrabold">Envíanos tu cotización</h2>
            <p className="text-sm text-muted-foreground">Completa el formulario y te responderemos al instante por WhatsApp.</p>
            {[
              ...
              <div key={f.n}>
                ...
                <input name={f.n} type={f.t} placeholder={f.p}
            ...
            <div>
              ...
              <textarea name="message" rows={4} placeholder="Describe los productos o servicios que necesitas..."
            ...
            <button type="submit" className="w-full inline-flex items-center justify-center gap-2 rounded-xl px-7 py-4 font-bold text-white bg-gradient-fire hover:opacity-90">
